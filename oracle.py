...
def structure_oracle(true_labels, pred_labels, true_K=None):
    """
    ...
    est_K = len(np.unique(pred_labels))
    
    metrics['K-hit'] = int(est_K == true_K)
...
def policy_oracle(customers, algo):
    ...
    for cust in customers:
        ...
        if est_seg.est_action != oracle_action:
            mistreated += 1
